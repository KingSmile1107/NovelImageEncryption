...
def sine_tent_cosine_map(r, x):
    if x < 0.5:
        return np.cos(np.pi * (r * np.sin(np.pi * x) + 2 * (1 - r) * x + 0.3))
    else:
        return np.cos(np.pi * (r * np.sin(np.pi * x) + 2 * (1 - r) * (1 - x) + 0.3))

def derivative_sine_tent_cosine_map(r, x):
    if x < 0.5:
        inner = np.pi * (r * np.sin(np.pi * x) + 2 * (1 - r) * x + 0.3)
        return -np.pi * np.sin(inner) * (np.pi * r * np.cos(np.pi * x) + 2 * (1 - r))
    else:
        inner = np.pi * (r * np.sin(np.pi * x) + 2 * (1 - r) * (1 - x) + 0.3)
        return -np.pi * np.sin(inner) * (np.pi * r * np.cos(np.pi * x) - 2 * (1 - r))
